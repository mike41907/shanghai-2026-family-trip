...
- 今天首頁：Day、目前行程、下一站、倒數、地址、營業時間、高德導航與滴滴叫車；首頁另有上海地鐵圖快速入口。
...
- 交通卡：步行、地鐵、滴滴、磁浮、飛機等交通段落；每段滴滴路線可一鍵開啟滴滴，並先複製目的地。
...
- 費用記帳：首頁直接顯示明顯的旅費卡，不必進管理模式；可記錄機票、住宿、餐飲、交通等台幣／人民幣支出、行程日、付款人、付款方式與備註，支援快速新增、依日期／Day／分類查看當日花費、付款人／分類統計、完整旅費 JSON／CSV 匯出與 JSON 匯入；目前篩選的旅費可開啟列印版並由系統儲存為 PDF。可用瀏覽器內 OCR 辨識支付寶、微信支付、美團外送截圖，再人工確認後入帳。
...
旅行工具中的費用與附件是本機資料；旅費現在可在首頁直接新增、編輯、刪除與匯出，不必進管理模式。清除網站資料或更換裝置前，請先匯出旅費並下載重要附件。旅費不會公開到家人的 `trip.json`，以免把家庭支出曝光；PDF 是由目前裝置開啟的列印版產生，可在系統列印視窗選擇「儲存為 PDF」，同樣不會上傳 GitHub。若未來需要家人共同查看，再將費用資料接到具權限控管的後端。任務清單則屬於正式行程資料，發布後家人可以唯讀查看。
...
高德導航使用官方 `https://uri.amap.com/search` URI；行動裝置會帶入原生 App 呼叫參數，無法開啟時回到網頁搜尋，避免舊版私有 scheme 觸發不支援功能提示。滴滴叫車會先複製該卡的目的地，再以 `diditaxi://` 開啟已安裝的滴滴；若未安裝則回到滴滴官網。滴滴未提供可在不持有 App ID／Token 的情況下安全使用的公開目的地預填 URI，因此仍須在滴滴內確認上車點與送出叫車。美團同樣先嘗試 deep link，並在卡片提供一鍵複製餐廳名稱的 fallback。沒有串接任何付費 API、雲端 API、遙測或 Token。
...
7. 不進管理模式，直接從首頁旅費卡新增台幣／人民幣機票、住宿、餐飲與交通，確認幣別分開統計、行程日、付款方式、日期／Day／分類篩選、付款人／分類統計、JSON／CSV 匯出與 JSON 匯入；選擇某日後確認當日總花費與「PDF」會只整理目前篩選資料，並在列印視窗可儲存為 PDF。
...
13. 在手機測試各滴滴交通段、景點或餐廳的「滴滴叫車」：確認目的地已複製、已安裝 App 時可開啟滴滴，未安裝時回到滴滴官網。
14. 在手機測試首頁緊急協助：確認 110／120／119 撥號連結、聯絡人本機保存、簡訊內容可編輯、複製與系統分享 fallback。
15. 在手機測試首頁地鐵圖入口：確認全螢幕檢視、橫向提示、放大／縮小／重設、滑動與關閉。
