...
- 上海地鐵圖：首頁可一鍵開啟全螢幕檢視，旅程頁內建 2024 官方路線圖，可放大、縮小、滑動與離線查看；手機會優先嘗試橫向顯示，另保留上海市交通委員會官方更新頁與本次磁浮、2＋13 號線的轉乘備選。
- 想吃：備選餐廳、分類／區域／營業時間、加入 Day 與時間、美團搜尋與店名複製。
- 旅程：飯店、航班、成員、飯店／機場／磁浮站快速導航。
- 家庭任務清單：護照、台胞證、網卡、充電器、藥品、退房等，可由管理者編輯、勾選並隨發布版本同步給家人。
- 費用記帳：首頁直接顯示明顯的旅費卡，不必進管理模式；可記錄機票、住宿、餐飲、交通等台幣／人民幣支出、行程日、付款人、付款方式與備註，支援快速新增、分類篩選、付款人／分類統計、旅費 JSON／CSV 匯出與 JSON 匯入；可用瀏覽器內 OCR 辨識支付寶、微信支付、美團外送截圖，再人工確認後入帳。
...
旅行工具中的費用與附件是本機資料；旅費現在可在首頁直接新增、編輯、刪除與匯出，不必進管理模式。清除網站資料或更換裝置前，請先匯出旅費並下載重要附件。旅費不會公開到家人的 `trip.json`，以免把家庭支出曝光；若未來需要家人共同查看，再將費用資料接到具權限控管的後端。任務清單則屬於正式行程資料，發布後家人可以唯讀查看。
...
付款截圖 OCR 是可選的本機功能：第一次使用會下載 Tesseract.js 的 OCR 核心與簡體中文／英文模型，之後在瀏覽器內辨識；辨識結果只會先填入記帳表單，必須人工確認後才寫入 `localStorage`。OCR 不保證完全正確，金額、幣別、日期與店家仍應以原始截圖核對。若需要保存原始截圖，請另外使用「票券與截圖」附件功能；附件仍只留在目前裝置。
...
一般開啟時是家人唯讀行程畫面，不會顯示正式行程的新增、編輯、刪除、拖曳或發布按鈕；旅費是獨立的本機工具，首頁可直接新增、編輯、刪除台幣／人民幣支出，或選擇拍照辨識，不必進管理模式。正式行程編輯仍須在右上角按「管理」；行程修改只會存到這台裝置的 `localStorage`，管理者關閉管理模式時，畫面仍回到最後已發布版本。
...
7. 不進管理模式，直接從首頁旅費卡新增台幣／人民幣機票、住宿、餐飲與交通，確認幣別分開統計、行程日、付款方式、分類篩選、付款人／分類統計、JSON／CSV 匯出與 JSON 匯入。
