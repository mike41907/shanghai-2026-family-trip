...
type ExpensePreset = Partial<Pick<ExpenseRecord, "title" | "category" | "paymentMethod" | "dayNumber" | "currency">>;
...
function App() {
  ...
  const handlePaymentOcrFile = async (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    event.target.value = "";
    if (!file) return;

    setOcrProgress({ progress: 0, status: "準備辨識" });
    try {
      const result = await recognizePaymentScreenshot(file, (progress, status) => {
        setOcrProgress({ progress: Math.max(0, Math.min(1, progress)), status });
      });
      const confidenceNote = typeof result.confidence === "number" ? ` · 信心 ${Math.round(result.confidence)}%` : "";
      setOcrReview({
        fileName: file.name,
        result,
        expense: {
          id: makeId("expense"),
          date: result.date ?? getDateKey(new Date()),
          title: result.merchant ?? `${result.source}付款`,
          amount: result.amountCny ?? 0,
          currency: "CNY",
          payer: "我",
      ...
      showToast("付款截圖辨識失敗，請改用清晰截圖或手動記帳。", "error");
    } finally {
      setOcrProgress(null);
    }
  };
  ...
  const handleExpenseImportFile = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const normalized = candidate.map(normalizeExpenseRecord);
      if (normalized.some((expense) => expense === undefined)) throw new Error("invalid");
      const imported = normalized.filter((expense): expense is ExpenseRecord => expense !== undefined).map((expense) => ({ ...expense, paymentMethod: expense.paymentMethod ?? "其他" as const }));
...
function TodayMapLauncher() {
  const [isOpen, setIsOpen] = useState(false);

  return (
    <>
      <button className="today-map-launcher" type="button" onClick={() => setIsOpen(true)} aria-label="開啟全螢幕上海地鐵圖">
        <span className="today-map-launcher-icon"><Map size={22} /></span>
        <span className="today-map-launcher-copy"><strong>上海地鐵圖</strong><small>2024 官方版 · 點擊全螢幕查看</small></span>
        <ChevronRight size={20} />
      </button>
      {isOpen && <MetroMapFullscreen onClose={() => setIsOpen(false)} />}
    </>
  );
}
...
function TodayPage({
  ...
  return (
    <div className="page-stack today-page">
      ...
      {travelTools && <TodayExpenseCard tools={travelTools} onAdd={onAddExpense} onOcr={onOcrExpense} onViewAll={onOpenExpenses} />}
...
function TodayExpenseCard({ tools, onAdd, onOcr, onViewAll }: { tools: TravelToolsData; onAdd: () => void; onOcr: () => void; onViewAll: () => void }) {
  const summary = summarizeExpenses(tools.expenses);
  return (
    <section className="today-expense-card">
      <div className="today-expense-heading">
        <div className="today-expense-title"><div className="info-card-icon expense-icon"><WalletCards size={21} /></div><div><span className="eyebrow">TRIP EXPENSES · LOCAL ONLY</span><h2>旅費記帳</h2><p>首頁即可記錄台幣、人民幣與付款截圖</p></div></div>
        <div className="today-expense-count">{tools.expenses.length} 筆</div>
      </div>
      <div className="today-expense-total"><div><span>人民幣</span><strong>{formatExpenseAmount(summary.byCurrency.CNY, "CNY")}</strong></div><div><span>新台幣</span><strong>{formatExpenseAmount(summary.byCurrency.TWD, "TWD")}</strong></div><span>{tools.expenses.length} 筆記錄</span></div>
      <div className="today-expense-actions"><button className="primary-button" type="button" onClick={onAdd}><Plus size={16} />新增花費</button><button className="secondary-button" type="button" onClick={onOcr}><FileText size={16} />拍照辨識</button><button className="secondary-button" type="button" onClick={onViewAll}><WalletCards size={16} />查看全部記錄</button></div>
    </section>
  );
}
    ...
    <>
      <section className="metro-map-card">
        <div className="section-heading">
          ...
          <div className="metro-map-heading-actions"><button className="metro-map-open-button" type="button" onClick={() => setFullscreenOpen(true)}><Maximize2 size={15} /><span>全螢幕</span></button><Map size={21} /></div>
        </div>
      ...
      {fullscreenOpen && <MetroMapFullscreen onClose={() => setFullscreenOpen(false)} />}
    </>
  );
}

function MetroMapFullscreen({ onClose }: { onClose: () => void }) {
  const [zoom, setZoom] = useState(1);
  ...
  useEffect(() => {
    ...
    const orientation = typeof screen !== "undefined"
      ? screen.orientation as ScreenOrientation & { lock?: (orientation: "landscape") => Promise<void>; unlock?: () => void }
      : undefined;
    if (orientation?.lock) {
  ...
  return (
    <div className="metro-map-fullscreen" role="dialog" aria-modal="true" aria-labelledby="metro-map-fullscreen-title">
      <div className="metro-map-fullscreen-header">
        <div className="metro-map-fullscreen-title"><Map size={21} /><div><h2 id="metro-map-fullscreen-title">上海地鐵圖</h2><span>2024 官方版</span></div></div>
        <div className="metro-map-fullscreen-controls" aria-label="全螢幕地鐵圖控制">
          <button type="button" className="metro-map-fullscreen-control" onClick={() => changeZoom(-0.5)} disabled={zoom <= 1} aria-label="縮小地鐵圖">−</button>
          <span>{Math.round(zoom * 100)}%</span>
          <button type="button" className="metro-map-fullscreen-control" onClick={() => changeZoom(0.5)} disabled={zoom >= 2.5} aria-label="放大地鐵圖">＋</button>
          <button type="button" className="metro-map-fullscreen-control metro-map-fullscreen-reset" onClick={() => setZoom(1)}>重設</button>
          <button type="button" className="metro-map-fullscreen-close" onClick={onClose}><X size={17} /><span>關閉</span></button>
        </div>
      </div>
      <div className="metro-map-fullscreen-body">
        <div className="metro-map-fullscreen-canvas" style={{ width: `${zoom * 100}%` }}>
          <img src={SHANGHAI_METRO_MAP_ASSET} alt="上海軌道交通網絡示意圖（2024 官方版）" draggable="false" />
        </div>
      </div>
      <div className="metro-map-fullscreen-footer"><span className="metro-map-orientation-hint">請將手機旋轉橫向查看；若裝置不支援自動旋轉，仍可滑動地圖。</span><span>可左右滑動查看 · 離線可用</span></div>
    </div>
  );
}
...
function formatExpenseTotals(totals: Record<ExpenseCurrency, number>): string {
  const formatted = [
    totals.CNY > 0 ? formatExpenseAmount(totals.CNY, "CNY") : undefined,
    totals.TWD > 0 ? formatExpenseAmount(totals.TWD, "TWD") : undefined
...
function ExpenseEditorDialog({ days, expense, preset, fileName, ocrResult, onClose, onSave }: { days: TripDay[]; expense?: ExpenseRecord; preset?: ExpensePreset; fileName?: string; ocrResult?: PaymentOcrResult; onClose: () => void; onSave: (expense: ExpenseRecord) => void }) {
  const [form, setForm] = useState<ExpenseRecord>(() => ({
    ...
    currency: expense?.currency ?? preset?.currency ?? "CNY",
  ...
  return <Modal title={ocrResult ? "確認 OCR 記帳" : expense ? "編輯旅費" : "新增旅費"} onClose={onClose}><form className="form-stack" onSubmit={(event: FormEvent) => { event.preventDefault(); if (!form.title.trim() || form.amount <= 0 || !form.payer.trim()) return; onSave({ ...form, title: form.title.trim(), payer: form.payer.trim(), note: form.note?.trim() || undefined, amount: Number(form.amount.toFixed(2)) }); }}>
    ...
    <div className="form-grid two-columns"><label>日期<input type="date" value={form.date} onChange={(event) => setField("date", event.target.value)} required /></label><div className="expense-amount-fields"><label>金額<input type="number" min="0.01" step="0.01" inputMode="decimal" value={form.amount || ""} onChange={(event) => setField("amount", Number(event.target.value))} placeholder="例如 128.50" required /></label><label>幣別<select value={form.currency} onChange={(event) => setField("currency", event.target.value as ExpenseCurrency)}><option value="CNY">人民幣（CNY）</option><option value="TWD">台幣（TWD）</option></select></label></div></div>
...
function OcrProgressDialog({ progress, onClose }: { progress: OcrProgressState; onClose: () => void }) {
  ...
  return <Modal title="辨識付款截圖" onClose={onClose}><div className="ocr-progress-content"><div className="ocr-progress-line"><strong>{formatPaymentOcrStatus(progress.status)}</strong><span>{percentage}%</span></div><div className="ocr-progress-track" role="progressbar" aria-label="付款截圖辨識進度" aria-valuemin={0} aria-valuemax={100} aria-valuenow={percentage}><span style={{ width: `${percentage}%` }} /></div><p>第一次使用會先下載開源 OCR 核心與中文模型；截圖會在目前裝置的瀏覽器內辨識，不會上傳到旅行行程。</p><small>辨識完成後仍需人工確認，才會加入本機記帳。</small></div></Modal>;
}
