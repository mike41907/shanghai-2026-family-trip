...
function TripInfoPage({ trip, managerMode, onEdit }: { trip: TripDocument; managerMode: boolean; onEdit: () => void }) {
  ...
  return (
    <div className="page-stack trip-page">
      ...
      <section className="info-card hotel-card"><div className="info-card-icon"><Hotel size={22} /></div><div className="info-card-content"><span className="eyebrow">HOTEL</span><h2>{hotel.name}</h2><p>{hotel.address}</p><AmapButton title={hotel.name} address={hotel.address} /></div></section>
      ...
      <section className="info-card members-card"><div className="info-card-icon"><Users size={22} /></div><div className="info-card-content"><span className="eyebrow">TRAVEL PARTY</span><h2>成員</h2><div className="member-list">{members.map((member) => <span key={member}>{member}</span>)}</div></div></section>
    </div>
  );
}
